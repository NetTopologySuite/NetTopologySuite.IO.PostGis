...
namespace NetTopologySuite.Geometries.Implementation
{
    ...
    public sealed class RawCoordinateSequence : CoordinateSequence, ISerializable
    {
        ...
        public bool HACK_TryGetSingleOrdinateGroup(out Memory<double> ordinateGroup, out int[] ordinateIndexes)
        {
            if (_rawData.Length != 1)
            {
                ordinateGroup = default;
                ordinateIndexes = null;
                return false;
            }

            ordinateGroup = _rawData[0].Array;
            ordinateIndexes = new int[_rawData[0].DimensionCount];

            var dimensionMap = _dimensionMap;
            for (int i = 0; i < dimensionMap.Length; i++)
            {
                ordinateIndexes[dimensionMap[i].DimensionIndex] = i;
            }
